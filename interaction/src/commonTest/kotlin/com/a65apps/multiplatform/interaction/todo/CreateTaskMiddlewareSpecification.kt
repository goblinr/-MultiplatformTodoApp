...
class CreateTaskMiddlewareSpecification : BaseMiddlewareSpecification<TodoAction, TodoState>() {
    ...
    override fun initMocks() {
        store = mockStore(state) {
            stubReduce(eq(state), eq(MainAction.NavigateForward(Screen.CREATE_TASK)), state)
            mainReducer = this
        }
    }
    ...
    @Test
    fun `it should navigate forward to CREATE_TASK screen on CreateTask action`() {
        val observer = bind()
        ...
        mainReducer.verifyReduce<MainState, MainAction>(
            eq(state), eq(MainAction.NavigateForward(Screen.CREATE_TASK)))
    }

    @Test
    fun `it should't respond to any action except CreateTask`() {
        val observer = bind()
